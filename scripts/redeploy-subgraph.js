...
async function main() {
    console.log("Starting Subgraph redeployment v0.2.6...");
    ...
    try {
        ...
        console.log("3. Deploying to Graph Studio (v0.2.6)...");
        console.log("Building and deploying subgraph version v0.2.6...");
        // Command for Subgraph Studio
        execSync(`npx graph deploy --studio medvault-subgraph --deploy-key ${process.env.GRAPH_DEPLOY_KEY} --version-label v0.2.6`, { 
            cwd: subgraphDir, 
            stdio: "inherit" 
        });

        console.log("Subgraph successfully deployed to v0.2.6");
    } catch (error) {
